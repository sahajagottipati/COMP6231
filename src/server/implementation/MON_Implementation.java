...
public class MON_Implementation extends DLMSPOA {
    ...
    final Logger log3= Logger.getLogger("Server");
	
    public MON_Implementation() throws RemoteException {
		super();
    ...
    public void sendMessage(int serverPort) {
		...
		try {
			...
			log3.info("Request message sent from the client to server with port number " + serverPort + " is: "+ new String(request.getData()));
			byte[] buffer = new byte[12];
			DatagramPacket reply = new DatagramPacket(buffer, buffer.length);
			aSocket.receive(reply);
			String status=new String(reply.getData());
			S+=status;
			log3.info("Reply received from the server with port number " + serverPort + " is: "+ status);
		...
		try {
			...
			while (true) {
				...
				if(datasplit.get(0).equals("Borrow Item")) {
					log3.info(datasplit.get(1)+datasplit.get(2)+datasplit.get(3));
					...
					if(check==0 || borrowItem.size()==0 || datasplit.get(2).substring(0, 3).equals(datasplit.get(3).substring(0,3)) ) {
							...
							log3.info(datasplit.get(1)+datasplit.get(2));
						  ...
						  if(status.equals("Book Available")) {
							  log3.info(datasplit.get(0)+datasplit.get(1));
								...
								if(check==0 || borrowItem.size()==0 || (datasplit.get(2).startsWith("MON")&&datasplit.get(3).startsWith("MON"))) {
	...
	public String addItem(String managerID, String itemID, String itemName, int quantity) {
		...
		if(managerID.matches("(MONM)([0-9]{4})")){
				log3.info(managerID+" requested to add "+itemID+" for "+itemName+" of "+quantity);
	...
	public String removeItem(String managerID, String itemID,int quantity) {
		// TODO Auto-generated method stub
		if(managerID.matches("(MONM)([0-9]{4})")){
				log3.info(managerID+" requested to remove "+itemID+" of quantity "+quantity);
	...
	public String borrowItem(String userID, String itemID, int numberOfDays) {
		...
		if(itemID.matches("(MON)([0-9]{4})")) {
			log3.info(userID+" requested to borrow "+itemID);
    		...
    		if(old_quantity==quantity && !borrow.containsKey(itemID)) {
    			...
    			if(old_quantity==0) {
    				...
    				if(quantity<=borrow_qty) {
    					...
    					status="Exceeded the maximum quantity, old quantity is "+old_quantity+" and borrow quantity is "+borrow.get(itemID).size();
    					log3.info(status);
    				}
    			}else {
     ...
     public String borrowItemFromLib(String userID,String itemID, HashMap<String,ItemDetails> lib, HashMap<String,List<String>> borrowItem) {
    	 ...
    	 if((lib.get(itemID) != null) && (lib.get(itemID).getQuantity()!=0)) 
    	 {
            ...
            log3.info(status);
            //log3.info("User borrowed an item with item id: "+itemID);
    	 } else {
      ...
      {
    	  if(managerID.matches("(MONM)([0-9]{4})")) {
    		  log3.info(managerID+" requested to display list of Items");
      ...
      {
    	  if(itemID.matches("(MON)([0-9]{4})")) {
    		  log3.info(itemID+" request to add "+userID+" to queue");
      ...
      {
    	  if(itemID.matches("(MON)([0-9]{4})")) {
    		  log3.info(userID+" requested to return "+itemID);
      ...
      public String returnItems(String userID, String itemID,HashMap<String, ItemDetails> lib,HashMap<String,List<String>> borrow,HashMap<String,List<String>> queue) 
      {
    	  ...
    	  if(borrow.containsKey(itemID) && borrow.get(itemID).contains(userID)) {
     		 ...
     		 status+=" Book is returned back to library and the updated quantity is "+lib.get(itemID).getQuantity();
     		 if(queue.containsKey(itemID) && queue.get(itemID).size()!=0) {
     			 user= queue.get(itemID);
     			 status+=borrowItem(user.get(0),itemID,30);
     			 user.remove(0);
    		 ...
    		 status="Book with item id "+itemID+" was not borrowed";
    	 }
    		 return status;
      }
      ...
      public String findItem(String userID, String itemName) {
    	      ...
    	      log3.info(userID+" requested to find items with item name "+itemName);
      ...
      public String checkBorrowList(String userID,String itemID) {
    	  log3.info(userID+" requested to check for "+itemID);
      ...
      public String checkBookAvailability(String itemID) {
    	  log3.info("Requested to check for "+itemID);
