...
public class MCG_Implementation extends DLMSPOA {
    ...
    final Logger log2= Logger.getLogger("Server");
	
    public MCG_Implementation() throws RemoteException {
		super();
    ...
    public void sendMessage(int serverPort) {
		...
		try {
			...
			log2.info("Request message sent from the client to server with port number " + serverPort + " is: "+ new String(request.getData()));
			byte[] buffer = new byte[12];
			DatagramPacket reply = new DatagramPacket(buffer, buffer.length);
			aSocket.receive(reply);
			String status=new String(reply.getData());
			S+=status;
			log2.info("Reply received from the server with port number " + serverPort + " is: "+ status);
    ...
    public void MCGserver() {
    	DatagramSocket aSocket = null;
		try {
			aSocket = new DatagramSocket(6788);
			byte[] buffer = new byte[120];
			log2.info("Server Started............");
			while (true) {
				...
				if(datasplit.get(0).equals("Borrow Item")) {
					log2.info(datasplit.get(1)+datasplit.get(2)+datasplit.get(3));
					...
					if(check==0 || borrowItem.size()==0 ) {
							...
							log2.info(datasplit.get(1)+datasplit.get(2));
						  ...
						  if(status.equals("Book Available")) {
							  log2.info(datasplit.get(0)+datasplit.get(1)+datasplit.get(2));
								...
								if(check==0 || borrowItem.size()==0 || (datasplit.get(2).startsWith("MCG")&&datasplit.get(3).startsWith("MCG"))) {
	...
	public String addItem(String managerID, String itemID, String itemName, int quantity) {
		...
		if(managerID.matches("(MCGM)([0-9]{4})")){
				log2.info(managerID+" requested to add "+itemID+" for "+itemName+" of "+quantity);
	...
	public String removeItem(String managerID, String itemID,int quantity) {
		// TODO Auto-generated method stub
		if(managerID.matches("(MCGM)([0-9]{4})")){
				log2.info(managerID+" requested to remove "+itemID+" of quantity "+quantity);
	...
	public String borrowItem(String userID, String itemID, int numberOfDays) {
		...
		if(itemID.matches("(MCG)([0-9]{4})")) {
			log2.info(userID+" requested to borrow "+itemID);
     ...
     public String borrowItemFromLib(String userID,String itemID, HashMap<String,ItemDetails> lib, HashMap<String,List<String>> borrowItem) {
    	 ...
    	 if((lib.get(itemID) != null) && (lib.get(itemID).getQuantity()!=0)) 
    	 {
            ...
            log2.info(status);
            //log2.info("User borrowed an item with item id: "+itemID);
    	 } else {
      ...
      {
    	  if(managerID.matches("(MCGM)([0-9]{4})")) {
    		  log2.info(managerID+" requested to display list of Items");
      ...
      {
    	  if(itemID.matches("(MCG)([0-9]{4})")) {
    		  log2.info(itemID+" request to add "+userID+" to queue");
      ...
      {
    	  if(itemID.matches("(MCG)([0-9]{4})")) {
    		  log2.info(userID+" requested to return "+itemID);
      ...
      public String returnItems(String userID, String itemID,HashMap<String, ItemDetails> lib,HashMap<String,List<String>> borrow,HashMap<String,List<String>> queue) 
      {
    	  ...
    	  if(borrow.containsKey(itemID) && borrow.get(itemID).contains(userID)) {
     		 ...
     		 status+=" Book is returned back to the library and updated quantity is "+lib.get(itemID).getQuantity();
     		 if(queue.containsKey(itemID) && queue.get(itemID).size()!=0) {
     			 user= queue.get(itemID);
     			 status+=borrowItem(user.get(0),itemID,30);
     			 user.remove(0);
      ...
      public String findItem(String userID, String itemName) {
    	      ...
    	      log2.info(userID+" requested to find items with item name "+itemName);
      ...
      public String checkBorrowList(String userID,String itemID) {
    	  log2.info(userID+" requested to check for "+itemID);
      ...
      public String checkBookAvailability(String itemID) {
    	  log2.info("Requested to check for "+itemID);
